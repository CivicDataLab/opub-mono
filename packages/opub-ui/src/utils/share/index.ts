 ...
 *
 * This hook provides several utility functions for converting React components to SVG, downloading files, converting SVG to PNG, and more.
 *
 ...
 *
 * @example
 * const { reactToSVG, downloadSvgAsPng } = useScreenshot();
 * const svg = await reactToSVG(<MyComponent />, { width: 800, height: 600 });
 * downloadSvgAsPng(svg, { scale: 2 }, 'MyComponent.png');
 */
export const useScreenshot = ({
   ...
   */
  const svgToPngURI = (svg: string): Promise<string> =>
   ...
   */
  async function shareImage(dataURI: string) {
